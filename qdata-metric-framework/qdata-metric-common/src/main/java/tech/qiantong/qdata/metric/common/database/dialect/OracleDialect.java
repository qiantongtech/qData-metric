...
import tech.qiantong.qdata.metric.common.database.utils.DatabaseUtil;
...
public class OracleDialect extends AbstractDbDialect {
    ...
    private List<String> generateOracleCreateSql(String tableName, String tableComment, List<DbColumn> columns) {
        ...
        for (DbColumn col : columns) {
            ...
            if (tech.qiantong.qdata.metric.common.utils.StringUtils.hasText(col.getDataDefault())) {
...
//                }
        ...
        // 表注释
        if (tech.qiantong.qdata.metric.common.utils.StringUtils.hasText(tableComment)) {
            String tableCmt = "COMMENT ON TABLE " + tableName + " IS '" + DatabaseUtil.escapeSingleQuotes(tableComment) + "'";
            sqlList.add(tableCmt);
        }
        // 字段注释
        for (DbColumn col : columns) {
            if (tech.qiantong.qdata.metric.common.utils.StringUtils.hasText(col.getColComment())) {
